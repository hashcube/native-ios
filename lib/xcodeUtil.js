...
Project.prototype.installModule = function (modulePath, config) {
  ...
  if (config.userBuildSettings) {
    var projectConfig = this._project.pbxXCBuildConfigurationSection(),
      projectKeys = Object.keys(projectConfig);

    for(var setting in config.userBuildSettings) {
      for (var i = 0; i < projectKeys.length; i++) {
        var projectKey = projectKeys[i],
          buildSettings = projectConfig[projectKey].buildSettings;
        if (buildSettings) {
          buildSettings[setting] = config.userBuildSettings[setting];
        }
      }
    }
  }
