...
var IOS_ICON_SIZES = ['20', '40', '57', '60', '72', '76', '80','114', '120', '144', '152', '167', '180', '1024'];
var IOS_7_ICON_SIZES = ['76', '120', '152', '180'];
var SPLASHES = [
  {
    key: 'portrait480',
    outFile: 'Default.png',
  ...
  {
    key: 'portrait960',
    outFile: 'Default@2x.png',
    outSize: '640x960'
  },
  {
    key: 'portrait1024',
    outFile: 'Default-Portrait~ipad.png',
    outSize: '768x1024'
  },
  {
    key: 'portrait1136',
    outFile: 'Default-568h@2x.png',
    outSize: '640x1136'
  },
  {
    key: 'portrait1334',
    outFile: 'Default-375w-667h@2x~iphone.png',
    outSize: '750x1334'
  },
  {
    key: 'portrait2048',
    outFile: 'Default-Portrait@2x~ipad.png',
    outSize: '1536x2048'
  },
  {
    key: 'portrait2208',
    outFile: 'Default-414w-736h@3x~iphone.png',
    outSize: '1242x2208'
  },
  {
    key: 'portrait2436',
    outFile: 'Default-2436h@3x~iphone.png',
    outSize: '1125x2436'
  },
  {
    key: 'landscape768',
    outFile: 'Default-Landscape~ipad.png',
    outSize: '1024x768'
  },
  {
    key: 'landscape1242',
    outFile: 'Default-Landscape@3x.png',
    outSize: '2208x1242'
  },
  {
    key: 'landscape1536',
    outFile: 'Default-Landscape@2x~ipad.png',
    outSize: '2048x1536'
  }
];
      ...
      if (iconPath) {
        ...
        tasks.push(exists(iconPath)
          .then(function (exists) {
            ...
            !isIOS7Icon && logger.log('Icons: Copying', 
              appPathToString(iconPath), 'to', appPathToString(targetOldPath));
...
exports.copyStickers = function (api, app, config) {
  var logger = api.logging.get('ios-build');
  var img_path = app.manifest.ios.stickers.path;
  var sticker_path = path.join(app.paths.root, img_path);
  var icon_path = path.join(img_path, 'icon');
  var sticker_target = path.join(config.xcodeProjectPath, 'stickers/stickers.xcassets/Sticker Pack.stickerpack');
  var icon_target = path.join(config.xcodeProjectPath, 'stickers/Stickers.xcassets/iMessage App Icon.stickersiconset');
  var tasks = [];
  var general_info = {
    version: 1,
    author: 'xcode'
  ...
  var sticker_list = fs.readdirSync(sticker_path);

  sticker_list.forEach(function(sticker){
    if (path.extname(sticker) === '.png') {
      var source_path = path.join(sticker_path, sticker);
      var target = path.join(sticker_target, sticker.split('.')[0] + '.sticker');
      var sticker_obj = {
        info: general_info,
        properties: {
          filename: sticker
      ...
      tasks.push(
        copyFile(source_path, path.join(target, sticker))
        .then(fs.writeFile.bind(fs, path.join(target, 'Contents.json'), JSON.stringify(sticker_obj)))
      );
    }
  });

  sticker_data.stickers = sticker_images;
  tasks.push(fs.writeFile(path.join(sticker_target, 'Contents.json'), JSON.stringify(sticker_data)));

  var icon_list = fs.readdirSync(icon_path);

  icon_list.forEach(function(icon, i){
    if (path.extname(icon) === '.png') {
      var info = icon.split('.')[0].split('_');
      var file_name = 'icon_' + i + '.png';
      if (info.length > 2) {
        var icon_obj = {
          size: info[1],
          idiom: info[0],
        ...
        if (icon_obj.idiom === 'universal' || icon_obj.idiom === 'ios-marketing') {
          icon_obj.platform = 'ios';
        }
        icon_images.push(icon_obj);
        tasks.push(copyFile(path.join(icon_path,icon), path.join(icon_target, file_name)));
      } else {
        logger.warn(icon + ': naming is not proper, ex: device_size_scale.png');
      }
    }
  });

  icon_data.images = icon_images;
  tasks.push(fs.writeFile(path.join(icon_target, 'Contents.json'), JSON.stringify(icon_data)));
