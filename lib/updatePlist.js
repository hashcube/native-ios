...
var replaceObjectRecursively = function (plistObj, manifest, plistConfig, key) {
  var value;

  Object.keys(plistConfig).forEach(function (nested_key) {
    var config_val = plistConfig[nested_key];

    if (typeof config_val === 'object' && !(config_val instanceof Array)) {
      key.push(nested_key);
      replaceObjectRecursively(plistObj, manifest, plistConfig[nested_key], key);
    } else {
      value = getPlistConfigValue(plistConfig, manifest, nested_key);
      obj.setVal(plistObj, key.concat(nested_key).join('.'), value);
      return;
    }
    key.pop();
  });
...
var copyKeysToPlist = function(plistObj, manifest, plistConfig) {
  Object.keys(plistConfig).forEach(function(key) {
    ...
    if (typeof plistConfig[key] === "object" && !(plistConfig[key] instanceof Array)) {
      /* For replacing nested keys
       Example: {foo:{"key": {"nested_key": "$(key)"}}}
      */
      replaceObjectRecursively(plistObj, manifest, plistConfig[key], [key]);
      return;
    } else {
      value = getPlistConfigValue(plistConfig, manifest, key);
    }
