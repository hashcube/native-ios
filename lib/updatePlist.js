...
var copyKeysToPlist = function(plistObj, manifest, plistConfig) {
  Object.keys(plistConfig).forEach(function(key) {
    ...
    // For replacing keys in nested JSON values
    if (typeof plistConfig[key] === "object" && !(plistConfig[key] instanceof Array)) {
      Object.keys(plistConfig[key]).forEach(function (nested_key) {
        value = getPlistConfigValue(plistConfig[key], manifest, nested_key);
        if (value === undefined) {
          return;
        }
        obj.setVal(plistObj, key + "." + nested_key, value);
        value = undefined;
      });
    } else {
